...
def vrrec(args=None):
    usage = \
            """
            vrrec ("V over R") is a comparison routine for precover.py. In this case the
            period is found by the traditional violet over red (V/R) flux ratio method in
            which one simply computes the ratio of blue to red-shifted flux. The routine
            operates like precover otherwise, i.e. Monte Carlo datasets are created and
            the best period from each is measured and reported. In this case just a single
            column of periods is sent to the output file.
            """
    ...
    parser.add_argument('vmax',   type=float, help='maximum velocity to include in V/R sums')
